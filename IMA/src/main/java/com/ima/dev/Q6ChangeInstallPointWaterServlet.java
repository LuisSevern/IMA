	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if (WoRefPrefix == 1) {
				...
				if (addSuffix) {
					WoRefNumber = Long
							.parseLong(request.getParameter("waterWoNumber") + "0" + request.getParameter("suffix"));
				} else {
					WoRefNumber = Long.parseLong(request.getParameter("waterWoNumber"));
				}
			}
		...
		try {
			if (WoRefPrefix2 == 1) {
				...
				if (addSuffix2) {
					WoRefNumber2 = Long
							.parseLong(request.getParameter("waterWoNumber2") + "0" + request.getParameter("suffix2"));
				} else {
					WoRefNumber2 = Long.parseLong(request.getParameter("waterWoNumber2"));
				}
			}
			...
			request.setAttribute("errormsg", "Error Message: The format of the Work Order "
					+ request.getParameter("waterWoNumber2") + " is wrong.");
			dispatcher = request.getRequestDispatcher("Q6changeinstallpointwater.jsp");
		}
