		...
		try {
			...
			if (meterExists != 0) {
				...
				request.setAttribute("result",
						"1. Error updating meter.  Not Allowed - Meter Number already exists in AMM.");
				...
				if (updateWaterMeter == true) {
					boolean updateWaterAsset = dbconn.updateWMSerialAsset(
							dbconn.getInstallPointFromInstallPlace(placeID), newMeterSerial, oldMeterSerial);
					...
					boolean updateInstPointWaterMeter = dbconn.updateInstallPointWaterMeterBySerial(
							dbconn.getInstallPointFromInstallPlace(placeID), oldMeterSerial, newMeterSerial);
					dbconn.closeConnection();
					...
					// error page here
					request.setAttribute("result",
							"2. Error updating meter.  Not Allowed - Meter Number already exists in AMM.");
					dbconn.closeConnection();
					RequestDispatcher dispatcher = request.getRequestDispatcher("error.jsp");
					dispatcher.forward(request, response);
				}
