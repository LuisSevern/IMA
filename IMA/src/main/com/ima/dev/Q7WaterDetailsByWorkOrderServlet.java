...
public class Q7WaterDetailsByWorkOrderServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			{
				if (addSuffix){
					WoRefNumber = Long.parseLong(request.getParameter("woRefNumber")+"0"+request.getParameter("suffix"));				
				} else {
					WoRefNumber = Long.parseLong(request.getParameter("woRefNumber"));				
				}
			}	
		} catch(NumberFormatException e){
			request.setAttribute("errormsg","Error Message: The format of the Work Order " + request.getParameter("woRefNumber") + " is wrong.");
		...
		try{		
			...
			{
				if (addSuffix2){
					WoRefNumber2 = Long.parseLong(request.getParameter("woRefNumber2")+"0"+request.getParameter("suffix2"));				
				} else {
					WoRefNumber2 = Long.parseLong(request.getParameter("woRefNumber2"));				
				}
			}
		} catch (NumberFormatException e){
			request.setAttribute("errormsg","Error Message: The format of the Work Order " + request.getParameter("woRefPrefix2") + " is wrong.");
			dispatcher = request.getRequestDispatcher("Q7swapwmmiumodule.jsp");
		}
		...
		try {		
			...
			if(woInfo.getWorkorder()!=null && woInfo2.getWorkorder()!=null && !woInfo.getWorkorder().contains(woInfo2.getWorkorder()))
			{
